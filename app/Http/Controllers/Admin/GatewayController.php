...
class GatewayController extends Controller
{
    public function index() {
        ...
        $data['elsaraf'] = PaymentGateway::find(20);
    ...
    public function elsarafUpdate(Request $request) {
        $elsaraf = PaymentGateway::find(20);
        $elsaraf->status = $request->status;
        $information = [];
        $information['merchantsecret'] = $request->merchantsecret;
        $elsaraf->information = json_encode($information);

        $elsaraf->save();

        $request->session()->flash('success', "Elsaraf informations updated successfully!");
